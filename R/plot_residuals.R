...
plot_residuals = function( Lat_i, Lon_i, TmbData, Report, Q, savedir=getwd(),
  ...
  sum_obs_xt = tapply( TmbData$b_i[which_pos], INDEX=list(TmbData$s_i[which_pos],TmbData$t_i[which_pos]), FUN=sum )
  sum_exp_xt = tapply( pred_y, INDEX=list(TmbData$s_i[which_pos],TmbData$t_i[which_pos]), FUN=sum )
  var_exp_xt = tapply( var_y, INDEX=list(TmbData$s_i[which_pos],TmbData$t_i[which_pos]), FUN=sum )
