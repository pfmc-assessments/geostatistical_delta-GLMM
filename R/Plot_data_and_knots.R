...
#' @export
Plot_data_and_knots = function( Extrapolation_List, Spatial_List, Data_Geostat, PlotDir=paste0(getwd(),"/"), Plot1_name="Data_and_knots.png", Data_name="Data_by_year.png", ...){

  # Plot data and grid
  png( file=paste0(PlotDir,Plot1_name), width=6, height=6, res=200, units="in")
    par( mfrow=c(2,2), mar=c(3,3,2,0), mgp=c(1.75,0.25,0) )
    plot( Extrapolation_List$Data_Extrap[which(Extrapolation_List$Extrap_Area_km2>0),c('Lon','Lat')], cex=0.01, main="Extrapolation (UTM)" )
    maps::map( "world", add=TRUE )
    plot( Extrapolation_List$Data_Extrap[which(Extrapolation_List$Extrap_Area_km2>0),c('E_km','N_km')], cex=0.01, main="Extrapolation (North-East)" )
