...
Spatial_Information_Fn = function( n_x, Lon_i, Lat_i, LON_intensity=Lon_i, LAT_intensity=Lat_i, Extrapolation_List, Method="Mesh", grid_size_km=50, grid_size_LL=1, ... ){
  ...
  if( Method %in% c("Mesh","Grid") ){
    if( is.numeric(Extrapolation_List$zone) ){
      ...
      loc_intensity = SpatialDeltaGLMM::Convert_LL_to_UTM_Fn( Lon=LON_intensity, Lat=LAT_intensity, zone=Extrapolation_List$zone, flip_around_dateline=Extrapolation_List$flip_around_dateline )                                                         #$
      loc_intensity = cbind( 'E_km'=loc_intensity[,'X'], 'N_km'=loc_intensity[,'Y'])
    }else{
      loc_i = SpatialDeltaGLMM::Convert_LL_to_EastNorth_Fn( Lon=Lon_i, Lat=Lat_i, crs=Extrapolation_List$zone )
      loc_intensity = SpatialDeltaGLMM::Convert_LL_to_EastNorth_Fn( Lon=LON_intensity, Lat=LAT_intensity, crs=Extrapolation_List$zone )
    }
    # Bounds for 2D AR1 grid
    ...
    # Calculate k-means centroids
    Kmeans = SpatialDeltaGLMM::Calc_Kmeans(n_x=n_x, loc_orig=loc_intensity[,c("E_km", "N_km")], ... )
