...
Plot_range_quantiles = function( Data_Extrap, Report, NN_Extrap, Year_Set=NULL, Prob_vec=c(0.10,0.5,0.90), FileName_Quantiles=paste0(getwd(),"/Range_boundary.png") ){
  # Default inputs
  ...
  Data_Extrap_Range = cbind( Data_Extrap[,c('Lat','Lon','N_km','E_km')], 'Include'=ifelse(Data_Extrap[,'Area_km2']>0, TRUE, FALSE), "Year"=Report$Index_xtl[NN_Extrap$nn.idx,,1] )
  ...
  png( file=FileName_Quantiles, width=6.5, height=6.5, res=200, units="in")
    ...
    for(z in 1:4){
      ...
      for(t in 1:nrow(Range_Quantile)) Range_Quantile[t,] = calc_quantile( x=Data_Extrap_Range[,c('Lat','Lon','N_km','E_km')[z]], w=Data_Extrap_Range[,paste0('Year.',t)], prob=c(0.05,0.5,0.95), doplot=FALSE )
