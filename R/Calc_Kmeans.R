...
function(n_x, loc_orig, nstart=100, iter.max=1000, DirPath=NULL, Save_Results=TRUE){
  ...
  if( length(unique(paste(loc_orig[,1],loc_orig[,2],sep="_")))<=n_x ){
    ...
    print( "n_x less than n_unique so no calculation necessary" )
  }else{
    if( paste0("Kmeans-",n_x,".RData") %in% list.files(DirPath) ){
      load( file=paste0(DirPath,"/","Kmeans-",n_x,".RData") )
      print( paste0("Loaded from ",DirPath,"/","Kmeans-",n_x,".RData") )
    }else{
      ...
      print( paste0("Calculated and saved to ",DirPath,"/","Kmeans-",n_x,".RData") )
    }
  }
  return( Kmeans )
}
