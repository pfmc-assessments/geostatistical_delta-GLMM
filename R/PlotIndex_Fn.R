PlotIndex_Fn <-
function( PlotName="Index.png", DirName, TmbData, Sdreport, Year_Set=NULL, Years2Include=NULL, interval_width=1, strata_names, use_biascorr=FALSE, plot_legend=TRUE, rawdata=NULL, total_area_km2=NULL, ... ){
  # Fill in missing
  ...
  png( file=paste0(DirName,"/",PlotName), width=4, height=4, res=200, units="in")
    ...
    plot(1, type="n", xlim=range(Year_Set), ylim=ifelse(plot_legend==TRUE,1.25,1.05)*Ylim, xlab="Year", ylab="Abundance (metric tonnes)" )
    ...
    if(plot_legend==TRUE) legend( "top", bty="n", fill=c(ifelse(Calc_design==TRUE,"black",NULL),rainbow(TmbData[['n_l']])), legend=c(ifelse(Calc_design==TRUE,"Design-based",NULL),strata_names), ncol=2 )
    # Write to file
