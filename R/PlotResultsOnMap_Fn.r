  ...
  for( cI in 1:Ncategories){
    for(plot_num in plot_set){
      ...
      if(plot_num==3){
        ...
        if("dpred_ktp" %in% names(Report)) Mat_xt = Report$dpred_ktp[,,cI]
      }
      if(plot_num==4){
        ...
        if("D_xcy"%in%names(Report)) Mat_xt = log(Report$R2_xcy+quantile(Report$R2_xcy,0.25))[,cI,]
        if(any(c("dhat_ktp","dpred_ktp")%in%names(Report)))  stop("Not implemented for SpatialVAM")
      }
      if(plot_num==5){
        ...
        if("D_xcy"%in%names(Report)) Mat_xt = log(Report$D_xcy+quantile(Report$D_xcy,0.25))[,cI,]
        if(any(c("dhat_ktp","dpred_ktp")%in%names(Report)))  stop("Not implemented for SpatialVAM")
      }
      if(plot_num==6){
        ...
        if("D_xcy"%in%names(Report)) stop("Not implemented for VAST version >= 2.0.0")
        if(any(c("dhat_ktp","dpred_ktp")%in%names(Report)))  stop("Not implemented for SpatialVAM")
      }
      if(plot_num==7){
      ...
      if(plot_num==8){
        ...
        if("D_xcy"%in%names(Report)) Mat_xt = Report$P1_xcy[,cI,]
        if(any(c("dhat_ktp","dpred_ktp")%in%names(Report)))  stop("Not implemented for SpatialVAM")
      }
      if(plot_num==9){
        ...
        if("D_xcy"%in%names(Report)) Mat_xt = Report$P2_xcy[,cI,]
        if(any(c("dhat_ktp","dpred_ktp")%in%names(Report)))  stop("Not implemented for SpatialVAM")
      }
      if(plot_num==10){
        ...
        if("D_xcy"%in%names(Report)){
          ...
          Mat_xt = array( TMB::summary.sdreport(Sdreport)[which(rownames(TMB::summary.sdreport(Sdreport))=="log(Index_xcyl)"),], dim=c(dim(Report$D_xcy),dim(Report$Index_cyl)[3],2), dimnames=list(NULL,NULL,NULL,NULL,c('Estimate','Std. Error')) )[,cI,,1,'Std. Error']
        }
