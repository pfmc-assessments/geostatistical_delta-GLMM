...
function(Sim_Settings, Extrapolation_List, MakePlot=TRUE){
  ...
  loc_i = Extrapolation_List$Data_Extrap[s_i,c('E_km','N_km','Lat','Lon')]
  ...
  X_ij = as.matrix(Extrapolation_List$Data_Extrap[s_i,c('Depth_km','Depth_km2','Rock_dist_')])
  ...
  P1_i = beta1_t[t_i] + O1_i + E1_i + as.matrix(X_ij)%*%unlist(Sim_Settings[c('Depth_km','Depth_km2','Dist_sqrtkm')])
  R1_i = plogis( P1_i + Vessel_vyc[v_i[which(t_i==t)],t,1] )
  P2_i = beta2_t[t_i] + O2_i + E2_i + as.matrix(X_ij)%*%unlist(Sim_Settings[c('Depth_km','Depth_km2','Dist_sqrtkm')])
  ...
  if(MakePlot==TRUE){
    ...
    for(RespI in 1:5){
      ...
      for(t in 1:Sim_Settings[['Nyears']]){
        NN_Extrap = RANN::nn2( data=loc_i[which(t_i==t),c('E_km','N_km')], query=Extrapolation_List$Data_Extrap[,c('E_km','N_km')], k=1 )
      ...
      png(file=paste(DateFile,"True_",switch(RespI, "Pres","Pos","Dens","Pos_Rescaled","Dens_Rescaled"),".png",sep=""), width=5*MapSizeRatio['Width(in)'], height=2*MapSizeRatio['Height(in)'], res=200, units='in')
        ...
        for(t in 1:Sim_Settings[['Nyears']]){
          ...
          points(x=Extrapolation_List$Data_Extrap[,'Lon'], y=Extrapolation_List$Data_Extrap[,'Lat'], col=Col(n=10)[ceiling(f(Mat)[,t]*9)+1], cex=0.01)
  ...
  # Return stuff
  Return = list( "Data_Geostat"=Data_Geostat )
  if( MakePlot==TRUE ) Return[["True_Index"]] = True_Index
  return( Return )
}
