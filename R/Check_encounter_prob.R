...
#'
#' @return Nothing (but generates plots)
...
Check_encounter_prob = function( Report, Data_Geostat, cutpoints_z=seq(0,1,length=21), interval_width=1.96, DirName=paste0(getwd(),"/"),
  ...
  png( file=paste0(DirName,"/",PlotName), width=5, height=5, res=200, units="in")
    par( Par )
    plot( x=midpoints_z, y=freq_z, pch=20, cex=1.2, xlab="Predicted encounter probability", ylab="Observed encounter probability" )
    SpatialDeltaGLMM:::Plot_Points_and_Bounds_Fn( x=midpoints_z, y=mean_z, ybounds=mean_z%o%c(1,1)+sd_mean_z%o%c(-interval_width,interval_width), lwd=2, bounds_type="shading", col_bounds=rgb(1,0,0,0.2), col="red" )
