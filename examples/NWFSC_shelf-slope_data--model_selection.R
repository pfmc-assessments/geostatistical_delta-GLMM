...
###############
  ...
  Q_Config = c("Pass"=1)   # Include pass as a fixed effect? (0: don't include; 1: do include)
...
for(ConfigI in 1:length(ObsModel_Set)){
  ...
  # Data
  TmbData = Data_Fn("Version"=Version, "Aniso"=Aniso, "FieldConfig"=FieldConfig, "ObsModel"=ObsModel, "b_i"=Data_Geostat[,'Catch_KG'], "a_i"=Data_Geostat[,'AreaSwept_km2'], "v_i"=as.numeric(Data_Geostat[,'Vessel'])-1, "s_i"=NN$nn.idx[,1]-1, "t_i"=Data_Geostat[,'Year']-min(Data_Geostat[,'Year']), "a_xl"=a_xl, "X_xj"=X_xj, "Q_ik"=Q_ik, "MeshList"=MeshList)
