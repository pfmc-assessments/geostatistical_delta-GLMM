...
Type objective_function<Type>::operator() ()
{
  ...
  if(FieldConfig(0)==1) jnll += Tmp1(Omegainput1_s);
  //if(FieldConfig(1)==1) jnll_comp(1) = SEPARABLE(AR1(Epsilon_rho1),Tmp1)(Epsiloninput1_st);
  if(FieldConfig(1)==1){
    for(t=0;t<n_t;t++){
      if(t==0) jnll += Tmp1(Epsiloninput1_st.col(t));
      if(t>=1) jnll += Tmp1(Epsiloninput1_st.col(t)-Epsilon_rho1*Epsiloninput1_st.col(t-1));
    }
  }
  if(FieldConfig(2)==1) jnll += Tmp2(Omegainput2_s);
  //if(FieldConfig(3)==1) jnll_comp(3) = SEPARABLE(AR1(Epsilon_rho2),Tmp2)(Epsiloninput2_st);
  if(FieldConfig(3)==1){
    for(t=0;t<n_t;t++){
      if(t==0) jnll += Tmp2(Epsiloninput2_st.col(t));
      if(t>=1) jnll += Tmp2(Epsiloninput2_st.col(t)-Epsilon_rho2*Epsiloninput2_st.col(t-1));
    }
  }

  // Random effect probabilities
  for(v=0;v<n_v;v++){
    jnll -= dnorm( nu1_v(v), Type(0.0), SigmaV1, true );
    jnll -= dnorm( nu2_v(v), Type(0.0), SigmaV2, true );
    for(t=0;t<n_t;t++){
      jnll -= dnorm( nu1_vt(v,t), Type(0.0), SigmaVT1, true );
      jnll -= dnorm( nu2_vt(v,t), Type(0.0), SigmaVT2, true );
    }
  }
  ...
  if( Options_vec(2)!=0 ){
    for(t=1;t<n_t;t++){
      jnll -= dnorm( beta1_t(t), Beta_rho1*beta1_t(t-1) + Beta_mean1, exp(logsigmaB1), true );
      jnll -= dnorm( beta2_t(t), Beta_rho2*beta2_t(t-1) + Beta_mean2, exp(logsigmaB2), true );
    }
  }
