  ...
  for (int i=0;i<n_i;i++){
    ...
    if(ObsModel(0)==0 | ObsModel(0)==1 | ObsModel(0)==2 | ObsModel(0)==11 | ObsModel(0)==12){ 
      ...
      if( b_i(i) > 0 ){    // 1e-500 causes overflow on laptop
        ...
        if(ObsModel(0)==11) LogProb2_i(i) = log( 1e-250 + SigmaM(1)*dnorm(log(b_i(i)),P2_i(i)+log(a_i(i))-pow(SigmaM(0),2)/2, SigmaM(0), false) + (1-SigmaM(1))*dnorm(log(b_i(i)),P2_i(i)+log(a_i(i))+log(1+SigmaM(2))-pow(SigmaM(0),2)/2, SigmaM(0), false) ); // log-space
        if(ObsModel(0)==12) LogProb2_i(i) = log( 1e-250 + SigmaM(1)*dgamma(b_i(i), 1/pow(SigmaM(0),2), R2_i(i)*a_i(i)*pow(SigmaM(0),2), false) + (1-SigmaM(1))*dgamma(b_i(i), 1/pow(SigmaM(0),2), R2_i(i)*a_i(i)*(1+SigmaM(2))*pow(SigmaM(0),2), false) ); // shape = 1/CV^2, scale = mean/CV^2
      }else{
        LogProb2_i(i) = 0;
      }
    }
